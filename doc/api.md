...
Для заданий используется поле *priority*, которые может принимать следующие значения (от самого низкого к самому высокому): 
...
**sort** - сортировка списков. Поля сортировки: *creation_time*, *update_time*, *name*. Значение по умолчанию - сортировка по дате обновления по убыванию.

**filter** - фильтрация списков. Фильтрация происходит по полям *creation_time*, *update_time*, *name*. По умолчанию фильтрация не происходит.
...
name должен быть непустой строкой (c учётом *trim*). color может отсутствовать (по умолчанию 0).
...
**sort** - сортировка дел. Поля сортировки: *creation_time*, *update_time*, *name*, *priority*, *done*.
Значение по умолчанию - сортировка по дате обновления по убыванию.

**filter** - фильтрация дел. Фильтрация происходит по полям: *creation_time*, *update_time*, *name*, *priority*, *done*. По умолчанию фильтрация не происходит.
...
*name* должен быть непустой строкой (c учётом *trim*). *description" не обязательный параметр.
