...
с использованием: *=*(равно, при сравнении строк не учитывает регистр), *>*(больше), 
*<*(меньше), *>=*(больше или равно), *<=*(меньше или равно), *&*(И), *|*(ИЛИ), *!*(НЕ). Например:
    
    & > update_date 30-11-2020 = name 'Такой-то список дел'.
    
Для булевых значений используется значения *true*/*false* и используется только оператор *=*. Для строковых значений строки выносятся в одинарные скобки.
